...
Panel {
  ...
  // ---- Fork: universal scale ------------------------------------------------
  // Sets one scale across every enabled output. Edits are serialized: each
  // edit_profile round-trip returns an updated profile, and the next edit must
  // be built against that result, so pending edits queue and drain one at a
  // time from the edit_profile response handler. After the last edit lands
  // we kick the draft apply, which rides the daemon's confirm-revert overlay.
  property var universalScaleQueue: []
  property bool universalScaleActive: false

  function applyUniversalScale(fieldsByKey) {
    ...
    root.universalScaleQueue = keys.map(function(k) { return fieldsByKey[k] })
    root.universalScaleActive = true
    root.drainUniversalScaleQueue()
  }

  function drainUniversalScaleQueue() {
    if (!root.universalScaleActive) return
    if (root.universalScaleQueue.length === 0) {
      root.universalScaleActive = false
      Qt.callLater(function() { root.applyDraft() })
      return
    }
    if (root.editPending) return  // response handler will call us again
    var fields = root.universalScaleQueue[0]
    root.universalScaleQueue = root.universalScaleQueue.slice(1)
    var key = String(fields.output_key || "")
    delete fields.output_key
    root.editOutput(fields, key)
  }
  ...
  function handleMessage(line) {
    ...
    if (envelope.error) {
      ...
      if (root.universalScaleActive) {
        // Abandon the queue on error: a mid-sequence failure leaves the draft
        // partially edited; the user reviews lastError and the editor state.
        root.universalScaleQueue = []
        root.universalScaleActive = false
      }
    ...
    if (method === "status" || method === "subscribe") {
      ...
      root.editPending = false
      root.draftDirty = true
      if (root.universalScaleActive) Qt.callLater(function() { root.drainUniversalScaleQueue() })
      Qt.callLater(function() {
        root.normalizeWorkspaceCursor()
        if (root.activePage === "workspaces") root.ensureManualWorkspaceRules()
      })
      ...
      Column {
        ...
        Column {
          ...
          UniversalScaleControl {
            ...
            previewBusy: root.previewPending || root.previewTransaction !== ""
            onRequested: function(fieldsByKey) { root.applyUniversalScale(fieldsByKey) }
          }
